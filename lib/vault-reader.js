...
const STRATEGY_DOC = 'Career/Public Voice - Positioning and Editorial POV.md';
...
export async function readVaultContent() {
  ...
  // Optional: scan BAI vault for additional context (read-only)
  const BAI_VAULT_PATH = process.env.BAI_VAULT_PATH;
  if (BAI_VAULT_PATH && fs.existsSync(BAI_VAULT_PATH)) {
    const baiFiles = findRecentMarkdown(BAI_VAULT_PATH, cutoff)
      .slice(0, MAX_FILES_PER_SECTION * 2);
    for (const file of baiFiles) {
      const raw = readFile(file);
      if (!raw) continue;
      recentContent.push({
        path: 'bai/' + path.relative(BAI_VAULT_PATH, file),
        excerpt: raw.slice(0, EXCERPT_LENGTH),
      });
    }
  }
